...
#define Echo GPIO_Pin_7		//HC-SR04模块的Echo脚接GPIOA7
#define Trig GPIO_Pin_6		//HC-SR04模块的Trig脚接GPIOA6
...
int16_t sonar_mm(void)									//测距并返回单位为毫米的距离结果
{
	...
	GPIO_WriteBit(GPIOA,Trig,Bit_SET);						//输出高电平
	Delay_us(15);										//延时15微秒
	GPIO_WriteBit(GPIOA,Trig,Bit_RESET);						//输出低电平
...
float sonar(void)										//测距并返回单位为米的距离结果
{
	...
	GPIO_WriteBit(GPIOA,Trig,Bit_SET);					//输出高电平
	Delay_us(15);
	GPIO_WriteBit(GPIOA,Trig,Bit_SET);					//输出低电平
